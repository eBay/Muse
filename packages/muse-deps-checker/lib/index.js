  ...
  museCore: {
    pm: {
      beforeDeployPlugin: async (ctx, params) => {
        const depsManifest =
          await muse.storage.assets.getJson(decodeURIComponent(`p/${params.pluginName}/v${params.version}/dist/deps-manifest.json`));
        ...
        if (depsManifest) {
          ...
          if (deployedLibPlugins) {
            for (const libPlugin of deployedLibPlugins) {
              ...
              const libManifest =
                await muse.storage.assets.getJson(decodeURIComponent(`p/${parsedLibName}/v${libPlugin.version}/dist/lib-manifest.json`));
