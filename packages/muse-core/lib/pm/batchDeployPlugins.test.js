...
const testJsPlugin = {
  ...
  museCore: {
    pm: {
      batchDeployPlugins: jest.fn(),
      beforeBatchDeployPlugins: jest.fn(),
      afterBatchDeployPlugins: jest.fn(),
    },
  },
};
...
describe('Deploy plugin basic tests.', () => {
  ...
  it('Batch deploy plugins should work', async () => {
    ...
    await muse.pm.batchDeployPlugins({
      appName,
      envMap: {
        staging: [
          {
    ...
    expect(testJsPlugin.museCore.pm.batchDeployPlugins).toBeCalledTimes(1);
    expect(testJsPlugin.museCore.pm.beforeBatchDeployPlugins).toBeCalledTimes(1);
    expect(testJsPlugin.museCore.pm.afterBatchDeployPlugins).toBeCalledTimes(1);
  });
  ...
  it('Batch deployments including undeployment should work', async () => {
    ...
    expect(testJsPlugin.museCore.pm.batchDeployPlugins).toBeCalledTimes(2);
    expect(testJsPlugin.museCore.pm.beforeBatchDeployPlugins).toBeCalledTimes(2);
    expect(testJsPlugin.museCore.pm.afterBatchDeployPlugins).toBeCalledTimes(2);
  });
});
