...
module.exports = async (params) => {
    ...
    const depsManifest =
        await assets.getJson(decodeURIComponent(`p/${params.pluginName}/v${params.version}/dist/deps-manifest.json`));
    ...
    if (depsManifest) {
        ...
        if (deployedLibPlugins) {
            for (const libPlugin of deployedLibPlugins) {
                ...
                const libManifest =
                    await assets.getJson(decodeURIComponent(`p/${parsedLibName}/v${libPlugin.version}/dist/lib-manifest.json`));
