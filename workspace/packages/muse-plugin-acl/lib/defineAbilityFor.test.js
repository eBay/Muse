...
describe('Define Ability For tests', () => {
  ...
  it('Release owner can update description field', () => {
    const ability = defineAbilityFor({ username: 'releaseOwner' });

    // Release owner can update the description field
    expect(
      ability.can(
        'update',
        subject('Release', { owners: ['releaseOwner'], description: 'Old description' }),
      ),
    ).toBe(true);
  });

  it('Non-owner cannot update description field', () => {
    const ability = defineAbilityFor({ username: 'nonOwner' });

    // Non-owner cannot update the description field
    expect(
      ability.can(
        'update',
        subject('Release', { owners: ['releaseOwner'], description: 'Old description' }),
  ...
  it('Release owner cannot update restricted fields', () => {
    const ability = defineAbilityFor({ username: 'releaseOwner' });

    // Release owner cannot update restricted fields like "createdBy"
    expect(
      ability.can(
        'update',
        subject('Release', { owners: ['releaseOwner'], createdBy: 'releaseOwner' }),
        'createdBy',
      ),
    ).toBe(false);
  });
