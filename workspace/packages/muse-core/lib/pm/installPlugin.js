...
const installPlugin = async params => {
  ...
  try {
    ...
    if (!pkgJson.muse) throw new Error(`Package "${pluginName}" seems not to be a Muse plugin.`);
    ...
    if (releases.find(r => r.version === pkgJson.version)) {
      throw new Error(`Version ${pkgJson.version} already exists in Muse registry.`);
    }
    await releasePlugin({
      pluginName,
