...
if (['lib', 'normal'].includes(pluginType)) {
  ...
  [pkgJson.dependencies, pkgJson.devDependencies, pkgJson.peerDependencies].forEach((deps) => {
    Object.entries(deps || {}).forEach(([name, version]) => {
      ...
      // If it's a strict version, need to ignore pre release part.
      // For example, if version is 1.2.3-beta.1, we need to check against 1.2.3.
      // This is because Muse's shared modules only use the major.minor.patch version,
      if (semver.parse(version)) {
        versionToCheck = semver.coerce(version).version;
      }
      if (libs[name] && !libs[name].find((o) => semver.satisfies(o.version, versionToCheck))) {
