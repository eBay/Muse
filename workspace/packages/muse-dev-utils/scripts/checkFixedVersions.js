...
// lib plugins needs to check fixed deps versions.
if (['lib'].includes(pluginType)) {
  console.log('Checking fixed deps versions...');
  const libs = {};
  const distLibManifest = fs.readJsonSync(
    path.join(process.cwd(), `./build/dist/lib-manifest.json`),
  );
