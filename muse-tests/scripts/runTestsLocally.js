...
// Ensure the script is run from the muse-tests package
const pkgJson = fs.readJsonSync(path.join(cwd, 'package.json'));
assert(pkgJson.name === 'muse-tests', 'This script should be run from the muse-tests package.');
...
const dest = path.join(cwd, 'tmp/muse-repo/muse-tests');
